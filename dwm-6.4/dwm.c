...
//                        x = selmon->ww - TEXTW(stext) - sp;
			...
			x += blw + 3 * sp; //why does this work? I dont know. It fixes a weird awesomebar patch bug
			c = m->clients;
...
//              tw = TEXTW(stext) - lrpad + 2; /* 2px right padding */
		tw = TEXTW(stext) - sp + 3;
		drw_text(drw, m->ww - tw - 2 * sp, 0, tw, bh, 3, stext, 0);
	}
...
//		drw_text(drw, x, 0, w, bh, wdelta + lrpad / 2, (selmon->alttag ? tagsalt[i] : tags[i]), urg & 1 << i);
                if (selmon->alttag) {
        		drw_text(drw, x, 0, w, bh + 4, wdelta + lrpad / 2, tagsalt[i], urg & 1 << i); 
                } else {
...
//				drw_text(drw, x, 0, tabw - 2 * sp, bh, lrpad / 2, c->name, 0);
				drw_text(drw, x, 0, tabw, bh, 4, c->name, 0);
                                x += tabw;
			}
		} else {
			drw_setscheme(drw, scheme[SchemeNorm]);
...
{ 
	FILE *pp = popen("echo -e \"no\nrestart\nyes\" | dmenu -nocursor -p \"Quit DWM?\"", "r");
	if(pp != NULL) {
		char buf[1024];
